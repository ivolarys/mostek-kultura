...
def classify(events: list[Event], cfg: Config, cache: dict, use_llm: bool = True) -> dict:
    ...
    llm_ok = use_llm and bool(os.environ.get("ANTHROPIC_API_KEY"))
    if pending and not llm_ok:
        log.info("LLM classification skipped (%d pending, use_llm=%s, key=%s)",
                 len(pending), use_llm, bool(os.environ.get("ANTHROPIC_API_KEY")))
    if pending and llm_ok:
        ...
        for i in range(0, len(ids), BATCH):
            ...
            log.info("LLM classified %d/%d events", len(results), len(chunk))
