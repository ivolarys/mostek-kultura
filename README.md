...
- **Zdroj existujícího typu** (`galileo`, `antee_rss`, `public4u`, `goout`): přidej záznam do `sources` s `url`, `place` (výchozí obec) a `priority`. Pak `build --record --source <name> --no-llm` nahraje fixture pro testy.
